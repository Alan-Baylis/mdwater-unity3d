Shader "MdWater/MdWater"
{
	...
	SubShader
	{
		...
		Pass {
			CGPROGRAM
			#pragma enable_d3d11_debug_symbols			// 调试用 todo.ksh
			#pragma target 3.0							// VTF, 能去掉吗？ todo.ksh
			#pragma vertex vert
			...
			// noise在GPU生成
			#pragma multi_compile CPUNOISE CNOISE PNOISE SNOISE SNOISE_AGRAD SNOISE_NGRAD
			...
			#ifndef CPUNOISE
			float4 GenNoise(float2 inputUV)
			{
				const float epsilon = 0.0001;

				float2 uv = inputUV * 4.0 + float2(0.002, 0.005) * _Time.y;
				...
				{
					#if defined(THREED)
						float3 coord = float3(uv * s, _Time.y * 0.4); // todo.ksh 太他妈快了，这里可调
			...
			float CalcVertexHeight(float vx, float vy)
			{
				...
				float2 noiseUV = float2(vx / gw_np_size, vy / gw_np_size);
				#ifdef CPUNOISE
					float fHeight = tex2Dlod(_NoiseTex0, float4(noiseUV, 0, 0));
				#else
					float fHeight = GenNoise(noiseUV).x;
				#endif
